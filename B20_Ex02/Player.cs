...
namespace B20_Ex02
{
     class Player
     {
          ...
          {
              get => m_ETypeOfPlayer;
              set => m_ETypeOfPlayer = value;
          }

          public Pair?[] ComputerMove(List <Cell> io_AvailableCards,Dictionary<int,List<Pair>> io_SeenCards)
          {
            Pair?[] resultMove = new Pair?[2];


            foreach(List<Pair> item in io_SeenCards.Values)
            {
            ...
            if(resultMove[0] == null)
            {
                ...
                if(io_SeenCards.TryGetValue(chosenCard.CellContent, out List<Pair> seenMatches))
                {
